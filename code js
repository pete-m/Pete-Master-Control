...
window.onload = function() {
    ...
    log.innerHTML = "Script Loaded. Ready.";
    ...
    if (saved) {
        try { buffers = JSON.parse(saved); } catch(e) { console.log("Init clean."); }
    }
...
function switchBuffer(newPath) {
    // 1. Save whatever is in the textarea to the OLD buffer
    buffers[currentPath] = document.getElementById('MAIN_TEXT').value;
    
    // 2. Change the pointer
    currentPath = newPath;
    
    // 3. Load the NEW buffer into the textarea
...
async function batchInit() {
    ...
    const namesRaw = document.getElementById('INIT_REPO_NAME').value;
    if(!pat || !namesRaw) return alert("PAT and Names required.");
